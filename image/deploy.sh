...
function start {
    docker run -d --network host --restart unless-stopped --name cpblx_tcp $(get_image_name tcp)
    docker run -d --network host --restart unless-stopped --name cpblx_udp $(get_image_name udp)
    docker run -d --network host --restart unless-stopped --name cpblx_tls $(get_image_name tls)
}
